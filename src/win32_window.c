        ...
        case WM_INPUT:
        {
            ...
            switch (data->header.dwType)
            {
                ...
                case RIM_TYPEKEYBOARD:
                {
                    ...
                    //             1. wait for #658 to be merged, and use its IMEStatus flag to block key events (though that PR seem to send key events even when IME is active, differs from master)
                    //             2. handle WM_IME_NOTIFY message ourselves and update a flag
                    // TODO(hnosm) test AltGr key
                    // TODO(hnosm) Delete, PgUp, PgDown, etc. on windows virtual keyboard generates E0 D3 (P/2 set 1 break code)
                    // TODO(hnosm) App key (make code E0 5D) (break code E0 DD, generated by virtual keyboard)
